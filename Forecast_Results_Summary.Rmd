---
title: "United States Solar Electric Energy Consumption"
output: html_document
---
# Introduction
This script explores trends in monthly United States solar electric energy consumption. Data was collected by the U.S. Energy Information Administration (https://www.eia.gov/).
...
# Data Exploration
Convert consumption data to numeric, impute missing data with 0, and slice the data to remove time points before the first occurrence of solar energy consumption (where trillion_mmbtu > 0).
```{r data-cleaning}
solar$TRILLION_MMBTU <- as.numeric(solar$TRILLION_MMBTU)
solar$TRILLION_MMBTU[is.na(solar$TRILLION_MMBTU)] <- 0
solar <- solar[order(solar$DATE),]
first_value <- which.max(solar$TRILLION_MMBTU > 0)
solar <- solar %>% slice(first_value:n())
```
View the time series plot for solar energy consumption.
```{r plot-full-data}
plot(solar$DATE, solar$TRILLION_MMBTU, type = 'l')
```
...
ts_train <- ts(train_data$TRILLION_MMBTU,
...
ts_test <- ts(test_data$TRILLION_MMBTU,start = c(start_year_test, start_month_test), frequency=12)
```
...
seasonplot(ts_train)
```
The seasonal plot above shows that the most solar electric energy is consumed during the summer months. This trend is most likely due to an increase in air-conditioning usage during the hotter months of the year.
# Forecast Models
...
# Final Forecast
I can now apply the ETS model to predict United States solar electric energy consumption for the next 10 years.
```{r}
